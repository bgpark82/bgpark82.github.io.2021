...
1. JSP의 Servlet이 없는 경우
   - **JSP의 자바코드를 컴파일하여 Servlet 클래스 생성**
   - Servlet에 요청을 전달
...
JSP를 실행하는 뜻은 **JSP를 컴파일한 결과인 Servlet 클래스를 실행한다**는 뜻이다
...
- 데이터 전송 성능 향상
- JSP 실행도중 버퍼를 비우고 새로운 내용 전송 가능 (에러처리 등)
- 버퍼가 차기 전까지 헤더 변경 가능

![image-20220502191958611](https://raw.githubusercontent.com/bgpark82/image/master/images/image-20220502191958611.png)

Buffer는 `autoFlush` 옵션으로 Buffer에 일정크기가 차면 자동으로 클라이언트로 flush 해준다

만약 `autoFlush`가 `false`일 때 Buffer 크기 이상 넘어가면 Exception이 발생한다
...
`war`는 Web Application Archieve의 약자로 웹 어플리케이션을 묶어놓은 파일이다

`war`는 JDK의 `jar` 명령어로 묶을 수 있따
...
Cookie는 **웹 서버와 웹 브라우저 양쪽에서 생성**할 수 있다

JSP에서는 Cookie를 웹서버에서 생성해서 응답 헤더로 보낸다

웹 브라우저는 쿠키 저장소에 Cookie를 저장시켜 웹 서버에 요청을 보낼 때 헤더로 보낸다

웹 브라우저는 쿠키 저장소에 Cookie가 삭제될때까지 웹서버로 Cookie를 전송한다



쿠키를 구성하는 요소는 다음과 같다

- 이름 : 각 Cookie는 이름으로 구분하고 콤마(`,`), 새미콜론(`;`), 등호(`=`), 공백을 제외한 아스키 문자로 구성된다
- 값 : Cookie의 이름과 관련된 값
- 유효시간 : 웹 브라우저가 Cookie를 얼마동안 보관할지 결정 (별도의 시간이 없으면 브라우저 종료시 삭제)
- 도메인 : 웹 브라우저가 지정한 서버 도메인만 Cookie를 전송하도록 제한
- 경로 : 웹 브라우저가 지정한 서버 경로만 Cookie를 전송하도록 제한
...
기본적으로 Cookie는 **Cookie를 생성한 서버에만 전송된다**

같은 도메인을 사용하는 모든 서버에 Cookie를 보낼 떄는 도메인들을 지정하면 된다

`.somehost.com` 처럼 `.`으로 시작하는 경우 관련 도메인에 모두 Cookie를 전달할 수 있다. (`mail.somehost.com`, `www.somehost.com`)

`www.somehost.com`처럼 `www`로 시작하는 경우 특정 도메인에 대해서만 Cookie를 전송할 수 있다

도메인 지정 시 주의사항은 **현재 서버의 도메인과 상위 도메인에만 전달할 수 있다**

> 웹 브라우저가 타 도메인으로 지정한 Cookie를 받지 않는 이유
>
> **보안 문제** 때문이다
>
...
1. 서버는 로그인에 성공하면 특정이름의 Cookie 생성한다
2. 웹브라우저는 해당 Cookie를 저장한다
3. 웹브라우저에 해당 Cookie가 존재하면 로그인 상태라고 판단한다
4. 웹브라우저는 요청을 보낼 때마다 Cookie를 서버로 전송한다
5. 서버는 웹브라우저에게 받은 Cookie로 로그인한 사용자인지 매번 확인한다
...
웹 컨테이너는 **기본적으로 한 웹 브라우저마다 하나의 Session을 생성**한다
...
웹 브라우저마다 별도의 Session을 가지고 고유한 Session ID로 구분한다

서버는 브라우저에 Session ID를 전송한다

브라우저는 요청을 보낼 때마다 Session ID를 보낸다

**서버와 브라우저가 Session ID를 공유하는 방법으로 Cookie를 사용한다.**
...
| 이름    | 설명                                                         |
| ------- | ------------------------------------------------------------ |
| Request | **하나의 요청을 처리**하는데 사용되는 JSP 페이지 사이에 공유 |
| Session | 웹 브라우저의 **여러 요청을 처리**하는 JSP 페이지 사이에 공유 |
...
Session은 **마지막 접근 이후 일정 시간 내에 다시 Session에 접근하지 않으면 자동으로 Session을 종료한다**
...
1. 로그인에 성공하면 Session 객체의 속성에 로그인 정보를 기록한다
...
자바는 DBMS 종류와 관계없이 하나의 JDBC API를 사용하여 데이터베이스 작업을 처리할 수 있다
...
DBMS 종류와 관계없이 추상화된 JDBC API를 통해 데이터베이스에 접근한다

각 **DBMS는 자신에게 알맞은 JDBC 드라이버를 제공**하고 JDBC API는 JDBC 드라이버를 거쳐 DBMS와 통신한다
...
JDBC 드라이버에 하당하는 클래스들은 **해당 메소드를 통해 로딩될 때 자동으로 JDBC 드라이버로 등록된다**
...
데이터베이스 커넥션은 JDBC로 데이터베이스와 연결할 떄 사용된다
...
>
> 이러한 문제를 해결하기 위해 Connection Pool은 Connection의 연결이 끊어지지 않았는지 여부를 검사한다
>
...
Servlet Container는 **처음 Servlet 실행 시, Servlet 객체를 생성**한다
...
모델 1은 JSP만 이용한 단순한 모델이다

모델 1은 요청을 **JSP가 직접 처리**한다
...
사용자는 모든 요청을 Controller에 보낸다

Controller는 비즈니스 로직을 담당하는 Model을 사용한다

Controller는 사용자에게 보여줄 View를 선택한다

Model과 View는 분리되어 있으므로 서로에게 영향을 주지 않는다

즉, **비즈니스 로직을 처리하는 Model**과 **화면에 보여줄 View를 분리**한다

Controller는 **요청에 알맞은 Model**과 **사용자에게 보여줄 View를 선택**하고 **웹 어플리케이션 전반적으로 적용되는 기술**을 처리한다 
